...
function fillCharacterListSelect(select, lists, selected = 'default') {
  ...
  placeholder.value = '';
  placeholder.textContent = 'selecione uma lista';
  select.appendChild(placeholder);
  lists.filter(list => list.name !== 'default').forEach(list => {
    const option = document.createElement('option');
    option.value = list.name;
    option.textContent = list.name;
    select.appendChild(option);
  });
  select.value = selected !== 'default' && lists.some(list => list.name === selected) ? selected : '';
}
